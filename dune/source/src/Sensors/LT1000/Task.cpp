...
namespace Sensors
{
  ...
  namespace LT1000
  {
    ...
    static const unsigned c_rmc_fields = 11;
    ...
    {
      //TO DO: ACRESCENTAR VARIAVEIS NECESSARIAS PARA O RMC
...
//TO DO: O QUE FAZ ISTO? INITIALIZATION COMMANDS
      ...
      onResourceAcquisition(void)
      {
        ...
        {
          throw RestartNeeded(DTR(Status::getString(CODE_COM_ERROR)),4);
        }
      }

      //Função necessaria para conectar UART
      bool
      openSocket(void)
      {
        char addr[128] = {0};
...
//TO DO: O que é o initialization command?
      ...
      interpretSentence(std::vector<std::string>& parts)
      {
        ...
        {
          interpretROT(parts);
        } //TO DO: ADICIONADO PARA O LT1000
        else if (hasNMEAMessageCode(parts[0], "RMC"))
        {
          interpretRMC(parts);
        }
      ...
      //! TO DO: Interpret RMC sentence (Proprietary NMEA message that
      //! provides ......).
      //! @param[in] parts vector of strings from sentence.
      void
      interpretRMC(const std::vector<std::string>& parts)
      {
        ...
        if (readLatitude(parts[3],parts[4], m_fix.lat)
            && readLongitude(parts[5], parts[6], m_fix.lon)
            && readNumber(parts[9].substr(4,2), m_fix.utc_year)
            && readNumber(parts[9].substr(2,2), m_fix.utc_month)
            && readNumber(parts[9].substr(0,2), m_fix.utc_day)
            && readDecimal(parts[8], m_fix.cog)
            && readDecimal(parts[7], m_fix.sog))
        {
